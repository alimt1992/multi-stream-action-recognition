...
def train_model(model, train_dl, valid_dl, criterion, optimizer,
    ...
    if not os.path.exists('models_willow'):
        os.mkdir('models_willow')
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
        #    best_model_wts = model.state_dict()
        #    torch.save(best_model_wts, "./models/resnet50-spatial-epoch-{}-acc-{:.5f}.pth".format(epoch, best_acc))
            
        if val_loss < best_loss:
            best_loss = val_loss
            best_model_wts = model.state_dict()
            torch.save(best_model_wts, "./models_willow/resnet50-pose-epoch-{}-loss-{:.5f}.pth".format(epoch, best_loss))
