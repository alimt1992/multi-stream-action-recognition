...
class APMeter(meter.Meter):
    ...
    def reset(self):
        ...
        #self.scores = torch.Tensor.new_tensor(torch.FloatStorage(), torch.float64) 
        self.targets = torch.LongTensor(torch.LongStorage())
        #self.targets = torch.Tensor.new_tensor(torch.LongStorage(), torch.long)
        self.scores.requires_grad_(False)
        self.targets.requires_grad_(False)
